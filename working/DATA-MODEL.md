...
> 狀態：BUILD_FREEZE_READY / Phase 1 — Working Current Truth。本文是 NodeFF 共享資料模型的 Current Truth。它定義跨 Function 共用的 entity、identity、durability、relation、DB responsibility、privacy boundary 與未來 migration hooks。Function 專屬的 request/response payload、UI state、processing logic 仍由各 Fxx 文件定義。
